...
function draw() {
  ...
  if (scrollY > windowHeight / 3) {
    ...
    introVideo.pause();
    wrapperDiv.show();  // ✅ 전체 div 표시
  } else {
    canvas.style.opacity = "1";
    introVideo.loop();
    wrapperDiv.hide();  // ✅ 전체 div 숨김
  }
...
function showBlackOverlay() {
  ...
  introText.style('margin', '0 auto 60px auto'); // 아래 여백  
  //introText.style('width', '1800px'); // 가로로 길게
  //introText.style('background-color', 'black');
  ...
  imagePaths.forEach(path => {
    ...
    img.parent(overlay);
    img.style('width', 'min(250px, 30vw)');
  ...
  img.mousePressed(() => {
    overlay.remove(); // 오버레이 닫기
    showProject(path); // 해당 로고의 슬라이드쇼 보여주기
    });
  });
...
function showProject(logoPath) {
  ...
  if (logoPath === "logo1.png") {
    ...
    gameTextBox.style('top', '26%');
    gameTextBox.style('left', 'calc(50% + 250px)');

  } else if (logoPath === "logo2.png") {
    // 비디오는 iframe이므로 imgElem 스타일 적용 생략
    gameTextBox.style('top', '30%');
    gameTextBox.style('left', 'calc(50% + 250px)');

  } else if (logoPath === "logo3.png") {
    if (currentItem.type !== "video") {
      imgElem?.style('width', '100%');        // 원하는 비율로 조절
      imgElem?.style('height', '100%');
    ...
    gameTextBox.style('top', '20%');
    gameTextBox.style('left', 'calc(50% + 250px)');
  } else if (logoPath === "logo5.png") {
    gameTextBox.style('width', '710px');
